				...
				else
				{
					...
					if (player->getPoweringUp() && seconds > gameData->getAbilityChargeTime()) {
						...
						else if (it == ItemModelType::bananaPerfect)
						{
							...
							//if (x >= (gameData->getTileLayout())[0].size() * TILE_SIZE) x = (gameData->getTileLayout()[0]).size() * TILE_SIZE - TILE_SIZE / 2;
							//if (z >= gameData->getTileLayout().size() * TILE_SIZE) z = gameData->getTileLayout().size() * TILE_SIZE - TILE_SIZE / 2;
