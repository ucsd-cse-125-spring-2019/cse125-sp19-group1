...
enum class ItemModelType { KEY1 = 1, KEY2, KEY3, KEY4, KEY5, KEY6, KEY7, KEY8, KEY9, CAKE };

struct Gate
{
public:
	Gate(std::vector<ItemModelType> aKeys = std::vector<ItemModelType>({ ItemModelType::KEY1, ItemModelType::KEY2, ItemModelType::KEY3 }), int num = 0) : location(Location()), progress(0.0f), gateNum(num), validKeys(aKeys), isOpen(false) {}
	...
	bool isValidKey(ItemModelType aKey) { return std::find(validKeys.begin(), validKeys.end(), aKey) != validKeys.end(); }

	// Updates progress and removes the key so that duplicate keys cannot be used
	void updateProgress(ItemModelType aKey) {
		progress++;
